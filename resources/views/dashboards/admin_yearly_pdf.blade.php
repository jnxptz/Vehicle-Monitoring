...
<body>
    ...
    <table class="meta-table">
        <tr>
            ...
            <td><span class="label">Year:</span> {{ $year }}</td>
        </tr>
    </table>
    ...
    <table class="cards-table">
        <tr>
            <td class="card">
                ...
                <div class="card-value">{{ number_format($totalLiters, 2) }} L</div>
                <div class="card-sub">Full Year {{ $year }}</div>
            </td>
            <td class="card">
                <div class="card-label">Total Cost</div>
                <div class="card-value">&#8369;{{ number_format($totalCost, 2) }}</div>
                <div class="card-sub">Full Year {{ $year }}</div>
            </td>
        </tr>
    </table>

    {{-- ── PER-MONTH CARDS: 4 per row ── --}}
    @php
    ...
    <h2>Monthly Breakdown Detail</h2>
    <table class="data-table">
        ...
        <tfoot>
            <tr>
                ...
                <td class="tr"><strong>{{ number_format($totalLiters, 2) }}</strong></td>
                <td class="tr"><strong>&#8369;{{ number_format($totalCost, 2) }}</strong></td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="highlight-box">
        <span class="label">Highest Consumption Month:</span>
        {{ $highest['month'] ?? 'N/A' }}
        ({{ number_format($highest['liters'] ?? 0, 2) }} L)
    </div>

    {{-- ── TOP VEHICLES ── --}}
    <h3>Top 5 Vehicles by Fuel Consumption</h3>
            ...
            <tbody>
                @foreach($topVehicles as $tv)
                    <tr>
                        ...
                        <td>{{ $tv['vehicle']->plate_number }} &mdash; {{ $tv['vehicle']->bm?->name ?? '&mdash;' }}</td>
                        <td class="tr">{{ number_format($tv['liters'], 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    @else
        <p style="color:#999; font-size:10.5px; margin-top:6px;">No vehicle fuel data available.</p>
    @endif

    {{-- ── BUDGET ANALYSIS ── --}}
    <h2>Boardmember Budget Analysis</h2>
    <table class="data-table">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach($boardmembersData as $bm)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $bm['user']->name }}</td>
                    <td>{{ $bm['vehicle']?->plate_number ?? '&mdash;' }}</td>
                    <td class="tr">&#8369;{{ number_format($bm['yearlyBudget'], 2) }}</td>
                    <td class="tr">&#8369;{{ number_format($bm['totalUsed'], 2) }}</td>
                    <td class="tr">&#8369;{{ number_format($bm['remaining'], 2) }}</td>
                    <td class="tr">{{ $bm['usedPercent'] }}%</td>
                </tr>
            @endforeach
        </tbody>
    </table>
